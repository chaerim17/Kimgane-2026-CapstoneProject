...
std::filesystem::path GetModuleDirectory()
{
    ...
    const DWORD length = GetModuleFileNameW(nullptr, modulePath, _countof(modulePath));     // GetModuleFileNameW -> exe/dll의 전체 경로를 알려주는 함수, nullptr -> 지금 실행중인 exe
    ...
    return std::filesystem::path(modulePath).parent_path();     // parent_path()는 상위 폴더를 보내줌 C:\...\bin\x64\Debug\Kimgane.Client.exe -> C:\...\bin\x64\Debug
}
